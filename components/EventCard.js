...
function EventCard(prop) {
  ...
  return (
    <div className='inline-block p-1 my-6 mx-8'>
      <div className='flex flex-row  border-2 border-primaryDark rounded-xl text-white hover:shadow-lg hover:shadow-primary hover:scale-105 hover:duration-100' title={description!==null?description.substring(0,150):title}>


        <div className='lg:w-64 rounded-l-lg relative bg-white'>
          <div className='absolute p-1 top-2 left-2 h-8 w-8 z-10 bg-slate-500/50 rounded-full shadow-lg hover:scale-110'>{isFav?<AiFillHeart fill='red' size={'26'}/>:<AiOutlineHeart size={'24'}/>}</div>
          <img src={image} height={'200px'} width={'300px'} className="object-cover inset-0 w-full h-full align-middle border-none rounded-l-lg"/>
        </div>

        <div className='flex flex-col w-72 px-3 bg-slate-400/30 hover:bg-blend-darken'>

          <div className='text-2xl font-semibold mb-2 mt-3'>{title.substring(0,30)}</div>
          ...
          <div className='flex flex-row justify-between'>
            ...
            <div className='font-medium'>{(price===null || price===0)?'Free':"Rs. "+price}</div>
          </div>
        </div>
      </div>
    </div>
